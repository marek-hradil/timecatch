"""Split DriveLM's filtered scenes 80/20 for the LoRA fine-tuning pilot.

Mirrors scripts/split_finetune_clevrer.py — see that file for rationale.
Split is stratified by frame-length bucket so both halves have matching
length distributions. Train half is the SFT fine-tuning source; held-out
half is disjoint (by scene) and used only for post-training eval.

Reads:  datasets/drive_lm/scenes_filtered.json
Writes: datasets/drive_lm/scenes_finetune_train.json
        datasets/drive_lm/scenes_finetune_heldout.json

Usage:
    python3 scripts/split_finetune_drivelm.py
"""
...
SRC = ROOT / "datasets" / "drive_lm" / "scenes_filtered.json"
TRAIN_OUT = ROOT / "datasets" / "drive_lm" / "scenes_finetune_train.json"
HELDOUT_OUT = ROOT / "datasets" / "drive_lm" / "scenes_finetune_heldout.json"
