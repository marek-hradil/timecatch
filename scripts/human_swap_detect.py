"""
Run swap-detect inference on the exact stimuli that human annotators saw.
...
Config fields (in addition to standard model/dataset/slurm/system_prompt):
  human_manifest: path to human_subset/swap_detect_<dataset>.json
"""
import sys
import os
...
def main(cfg: dict) -> None:
    ...
    exp_name = f"human_swap_detect_{cfg['dataset']['name']}_{cfg['model']['name']}"
    results = Results(
        exp_name,
        cfg["model"]["name"],
        total=len(scenarios),
