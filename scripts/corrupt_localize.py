...
def corrupt_localize_pattern(n: int) -> str:
...
def main(cfg: dict) -> None:
    ...
    scenarios = CorruptDataset(base).sample_position(cfg.get("n"))
    exp_name = f"corrupt_localize_{cfg['dataset']['name']}_{cfg['model']['name']}"
    ...
    for i in range(0, len(scenarios), batch_size):
        ...
        answers = model.ask_batch(requests, pattern=corrupt_localize_pattern(max(len(s.frames) for s in batch)))
        for s, answer in zip(batch, answers):
            results.log(s.name, len(s.frames), s.position[0], answer)
